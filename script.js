...
function renderTray(room, roomState) {
  tray.innerHTML = "";
  selectedItemId = roomState.placed.includes(selectedItemId) ? null : selectedItemId;
...
function renderDropzones(room, roomState) {
  ...
  room.items.forEach(item => {
    ...
    zone.addEventListener("click", () => {
      if (selectedItemId === item.id && !roomState.placed.includes(item.id)) {
        placeItem(item.id);
      }
    });
...
function enableWipe(element, dustConfig) {
  ...
  const onMove = event => {
    ...
    if (lastY === null) lastY = event.clientY;
    const delta = Math.abs(event.clientX - lastX) + Math.abs(event.clientY - lastY) + 6;
    lastX = event.clientX;
    lastY = event.clientY;
    progress += delta;
